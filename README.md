...
`ZZ` 是一个非常轻量的对齐型 Skill，它的作用是让 Agent 在开始实质工作前，**强制**让Agent先复述它对你输入内容的理解，并在向你提问不确定的地方，对齐用户与Agent的认识|。

其实主要是每次都打一遍“请重复对我内容的理解……”太累了，不如直接调用 `/ZZ` 吧！精心挑选26个字母最后一个字符保证一定是最后一个skill，做到从**想起到调用完成只需要三步：**
